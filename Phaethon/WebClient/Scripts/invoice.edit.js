...
function products() {
    ...
    $("#newItemRow").click(function () {
        ...
        $("#itemTable tbody").append("<tr>" +
            ...
            "<input data-val='true' data-val-number='The field ID must be a number.' data-val-required='The ID field is required.' id='Elements_" + rowValue + "__Item_Product_ProductGroup_ID' name='Elements[" + rowValue + "].Item.Product.ProductGroup_ID' required='required' type='hidden'>" +
            "<input data-val='true' data-val-number='The field ID must be a number.' data-val-required='The ID field is required.' id='Elements_" + rowValue + "__Item_IncomingTaxGroup_ID' name='Elements[" + rowValue + "].Item.IncomingTaxGroup_ID' required='required' type='hidden'>" +
            ...
            "<td><input type='button' class='btn btn-danger btn-block' value='Delete' title='This button removes item, action cant be canceled.'></td>" +
            "</tr>");
            ...
            success: function (data) {
                if (data !== null) {
                    if (data.Items.length != 0) {
                        $("#Elements_" + rowValue + "__Item_IncomingPrice").val(data.Items[0].Price);
                    }
